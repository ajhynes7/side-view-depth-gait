...
def main():
    ...
    df_match_zeno = pd.merge(
        df_match, df_z_raw, left_on='Zeno', right_index=True)
    ...
    plt.figure()
    plt.scatter(df_k['step_length'], df_z['step_length'])
    plt.show()
...
if __name__ == '__main__':
    ...
    kinect_file = 'kinect_gait_metrics.csv'
    zeno_file = 'zeno_gait_metrics.csv'

    df_k_raw = pd.read_csv(os.path.join(results_dir, kinect_file), index_col=0)
    df_z_raw = pd.read_csv(os.path.join(results_dir, zeno_file), index_col=0)
